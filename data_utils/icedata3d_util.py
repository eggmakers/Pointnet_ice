...
def collect_point_label(ice_path, out_filename, file_format='numpy'):
    """
    读取每个txt文件（格式为 XYZ reflectance），根据文件名添加类别标签，
    最终保存为 XYZReflectanceLabel 格式的 .npy 文件。

    参数:
        ice_path: 包含多个.txt标注文件的文件夹路径
        out_filename: 保存的输出文件路径（含 .npy 扩展名）
        file_format: 保存格式，默认为 'numpy'
    """
    points_list = []

    for f in glob.glob(os.path.join(ice_path, '*.txt')):

        cls = os.path.basename(f).split('_')[0]
        print(f"Processing class: {cls}  from file: {f}")
        if cls not in g_classes:
            cls = 'clutter'  # 未知类归为 'clutter'

        points = np.loadtxt(f)  # Nx4: X Y Z reflectance
        labels = np.ones((points.shape[0], 1)) * g_class2label[cls]  # Nx1
        points_with_label = np.concatenate([points, labels], axis=1)  # Nx5: XYZ R L
        points_list.append(points_with_label)

    data_label = np.concatenate(points_list, axis=0)  # Mx5
    xyz_min = np.amin(data_label[:, 0:3], axis=0)
    data_label[:, 0:3] -= xyz_min  # 可选：平移，使最小坐标为 0

    if file_format == 'numpy':
        np.save(out_filename, data_label)
    elif file_format == 'txt':
        np.savetxt(out_filename, data_label, fmt='%.4f %.4f %.4f %.4f %d')
    else:
...
def collect_bounding_box(ice_paths, out_filename):
    ...
    Args:
        ice_paths: path to annotations. e.g. Area_1/office_2/Annotations/
    ...
    for f in glob.glob(os.path.join(ice_paths, '*.txt')):
        cls = os.path.basename(f).split('_')[0]
...
def collect_point_bounding_box(ice_path, out_filename, file_format):
    """ Compute bounding boxes from each instance in original dataset files on
        one room. **We assume the bbox is aligned with XYZ coordinate.**
    ...
    Args:
        ice_path: path to annotations. e.g. Area_1/office_2/Annotations/
...
if __name__ == '__main__':
    ...
    collect_point_label('/Users/josephyang/codefilder/Pointnet_ice/data/s3dis/Area_2/env_6_4_1/Annotations', os.path.join(os.path.join(ROOT_DIR, 'data/icedata_3d'), 'Area_2_env_6_4_1.npy'), 'numpy')
    # print(glob.glob(os.path.join('/Users/josephyang/codefilder/Pointnet_ice/data/s3dis/Area_2/env_6_4_1/Annotations', '*.txt')))