...
class IceDataset(Dataset):
    def __init__(self, split='train', data_root='data/', num_point=4096, test_split='Area_1', block_size=1.0, sample_rate=1.0, transform=None):
        super().__init__()
        self.num_point = num_point
        self.block_size = block_size
        self.transform = transform
        all_files = sorted(os.listdir(data_root))
        rooms = [f for f in all_files if f.endswith('.npy')]  # 你的数据为 .npy 文件

        if split == 'train':
            split_rooms = [r for r in rooms if test_split not in r]
        else:
            split_rooms = [r for r in rooms if test_split in r]
        ...
        for room_file in split_rooms:
            room_data = np.load(os.path.join(data_root, room_file))  # shape: N x 5
            points, labels = room_data[:, 0:4], room_data[:, 4].astype(int)
        ...
        room_idxs = []
        for i in range(len(split_rooms)):
            room_idxs.extend([i] * int(round(sample_prob[i] * num_iter)))
        self.room_idxs = np.array(room_idxs)
        print("Loaded {} samples for {} set.".format(len(self.room_idxs), split))
    ...
    def __getitem__(self, idx):
        ...
        if point_idxs.size >= self.num_point:
            selected_idxs = np.random.choice(point_idxs, self.num_point, replace=False)
        else:
            selected_idxs = np.random.choice(point_idxs, self.num_point, replace=True)

        selected_points = points[selected_idxs]
        selected_points[:, 0] /= self.room_coord_max[room_idx][0]
        selected_points[:, 1] /= self.room_coord_max[room_idx][1]
        selected_points[:, 2] /= self.room_coord_max[room_idx][2]
        selected_points[:, 3] /= 255.0  # intensity normalization
...
if __name__ == '__main__':
    data_root = 'D:\pn\Pointnet_ice\data\icedata_3d'
    num_point, test_area, block_size, sample_rate = 4096, 5, 1.0, 0.01

    point_data = IceDataset(split='train', data_root=data_root, num_point=num_point, test_area=test_area, block_size=block_size, sample_rate=sample_rate, transform=None)
